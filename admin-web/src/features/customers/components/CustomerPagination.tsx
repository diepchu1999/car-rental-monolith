...
export function CustomerPagination({
  ...
  return (
    <div className="customer-pagination card animate-fade-up">
      <div className="customer-pagination-summary">
        <span>
          Hiển thị <strong>{start}</strong>-<strong>{end}</strong> trong{" "}
          <strong>{total}</strong> khách hàng
        </span>
        <span className="customer-pagination-page">
          Trang {page}/{safeTotalPages}
        </span>
      </div>

      <div className="customer-pagination-controls">
        <label className="customer-page-size">
          <span>Số dòng</span>
          <select
            className="form-select customer-page-size-select"
            value={size}
            disabled={disabled}
        ...
        <div className="pagination-btns">
          ...
          {pageNumbers.map((pageNumber, index) =>
            pageNumber === "ellipsis" ? (
              <span className="customer-page-ellipsis" key={`ellipsis-${index}`}>
                ...
              </span>
            ) : (
              <button
