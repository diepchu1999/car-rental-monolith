...
export function VehiclePagination({
  ...
  return (
    <div className="vehicle-pagination card animate-fade-up">
      <div className="vehicle-pagination-summary">
        <span>
          Hiển thị <strong>{start}</strong>-<strong>{end}</strong> trong{" "}
          <strong>{total}</strong> xe
        </span>
        <span className="vehicle-pagination-page">
          Trang {page}/{safeTotalPages}
        </span>
      </div>

      <div className="vehicle-pagination-controls">
        <label className="vehicle-page-size">
          <span>Số dòng</span>
          <select
            className="form-select vehicle-page-size-select"
            value={size}
            disabled={disabled}
        ...
        <div className="pagination-btns">
          ...
          {pageNumbers.map((pageNumber, index) =>
            pageNumber === "ellipsis" ? (
              <span className="vehicle-page-ellipsis" key={`ellipsis-${index}`}>
                ...
              </span>
            ) : (
              <button
